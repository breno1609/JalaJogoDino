...
class Game:
    ...
    def show_menu(self):
        ...
        if self.death_count == 0:
            font = pygame.font.Font(FONT_STYLE, 22)
            text = font.render('Pressione any key to start', True, (0, 0, 0))
            text_rect = text.get_rect()
            text_rect.center = (half_screen_widht, half_screen_height)
            self.screen.blit(text, text_rect)
        else:
            self.screen.blit(ICON, (half_screen_widht - 20, half_screen_height - 140))
            #Mostrar mensagem de Restart press R, Score atingido e contador de mortes
            #Resetar a conategm do score, velocidade e vidas antes de começar outra partida
            #Método para abstrair as fontes 
